...
const CardsShowcase = () => {
  const [scrollIndex, setScrollIndex] = useState(6);
  const [selectedIndex, setSelectedIndex] = useState(6);
  const [transformStyle, setTransformStyle] = useState('rotateX(0deg) rotateY(0deg)');
  const displayAreaRef = useRef(null);
  ...
  return (
    <section className="bg-[#f5f7fa] py-10 sm:py-12 px-4 sm:px-6 flex justify-center items-center overflow-hidden min-h-[600px] sm:min-h-[650px]">
      <div className="w-full max-w-6xl flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
        ...
        <div className="flex flex-col items-center shrink-0 w-full md:w-auto">
          ...
          <button 
            onClick={handlePrev}
            className="text-[#6a0dad] hover:scale-125 transition-transform py-2 sm:py-3 px-2 cursor-pointer z-20 focus:outline-none"
            aria-label="Previous card"
          >
            <svg className="w-6 h-6 stroke-[#6a0dad] stroke-[3] fill-none" viewBox="0 0 24 24">
              <path d="M18 15l-6-6-6 6" strokeLinecap="round" strokeLinejoin="round"/>
          ...
          {/* Քարտերի Պատուհան */}
          <div className="h-[360px] sm:h-[410px] w-52 overflow-hidden relative">
            <div 
              className="flex flex-col gap-5 transition-transform duration-700 ease-in-out absolute w-full top-0"
              style={{
                transform: `translateY(-${translateYValue}px)`
            ...
            >
              {cardsData.map((card, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Քարտի Նկար */}
                    <div className={`w-36 sm:w-40 h-20 sm:h-24 flex items-center justify-center rounded-xl p-1 transition-all ${
                      isSelected ? 'scale-105' : 'scale-95'
                    }`}>
                      <img 
                        src={card.img} 
        ...
        <div 
          ...
          onMouseMove={handleMouseMove}
          onMouseLeave={handleMouseLeave}
          className="flex-1 flex flex-col items-center justify-center py-4 cursor-pointer [perspective:1000px] w-full"
        >
          <div 
            className="w-[280px] sm:w-[380px] md:w-[430px] h-[175px] sm:h-[235px] md:h-[265px] transition-all duration-300 ease-out [transform-style:preserve-3d]"
            style={{ transform: transformStyle }}
          >
            <img 
              src={activeCard.img} 
          ...
          <div className="w-[220px] sm:w-[300px] md:w-[350px] h-3 bg-[radial-gradient(ellipse_at_center,_rgba(0,0,0,0.12)_0%,_rgba(0,0,0,0)_70%)] mt-6 sm:mt-8 rounded-full" />
        </div>

        {/* 3. ԱՋ ՄԱՍ. Տեքստեր ու կետեր */}
        <div className="w-full md:w-[420px] flex flex-col items-center md:items-start text-center md:text-left gap-3 sm:gap-4 px-2 sm:px-0">
          
          {/* Վերնագիր */}
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#1f2937] tracking-tight">
            {activeCard.name}
          </h2>
          ...
          {/* Մանուշակագույն կետերով ցուցակ */}
          <ul className="flex flex-col gap-2.5 sm:gap-3 my-1 w-full">
            {activeCard.points.map((pt, i) => (
              <li key={i} className="flex items-start gap-3 text-left">
                <span className="w-2 h-2 rounded-full bg-[#6a0dad] mt-1.5 sm:mt-2 shrink-0" />
                <span className="text-xs sm:text-sm md:text-base text-gray-600 font-medium leading-snug">
                  {pt}
                </span>
              </li>
            ))}
          ...
          {/* Մանուշակագույն Կոճակ */}
          <button className="px-8 sm:px-10 py-3 bg-[#6a0dad] hover:bg-[#580a91] text-white font-semibold text-xs sm:text-sm rounded-full shadow-md shadow-[#6a0dad]/20 hover:shadow-lg transition-all duration-300 cursor-pointer mt-2 sm:mt-3">
            Մանրամասն
          </button>
        </div>
